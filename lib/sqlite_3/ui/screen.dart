...
class _ScreensDBState extends State<ScreensDB> {
  ...
  _searchItem(String value) async{
    List resultsearch = [];
    if (value.isEmpty) {
      searchItem =await MahsulotTable.obyekt.values.toList();
    } else {
      resultsearch = await MahsulotTable.obyekt.values
          .toList()
          .where((element) =>
